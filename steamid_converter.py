...
from . import root_path, headers, proxies, api
import re
import math
import requests
import cloudscraper
...
async def merge_steam_details(steamid):
    try:
        ...
        lastlogoff = datetime.datetime.fromtimestamp(steam_summary['lastlogoff']) if 'lastlogoff' in steam_summary.keys() else None # steam返回值有可能没有lastlogoff 很啥b
...
'''.strip()
    except ValueError as e: msg = f'转换失败...错误原因为：{e}'
...
async def get_steamid_from_username(user_name,is_d2_msid=False):
    ...
    try:
        if not is_d2_msid:
            d2_resp = await api.callapi(api.search_destiny_player(displayName=user_name, membershipType=3))
            d2_data = d2_resp.data
            membershipId = d2_data[0]['membershipId']
        else: membershipId = user_name
        msd_resp = await api.callapi(api.get_memberships_by_id(membershipId=membershipId,membershipType=3)) # 通过d2的membershipId获取BungieNet的ID
        ...
        for i in div_results:
            ...
            if '(ID: 7656' in data:
                steamid = data[-18:-1]
                if is_steamid64(steamid): return steamid
                else: continue
    except Exception as e:
        logger.error(f'GetSteamIdFromUsername error, reason: {e}')
        return None
...
def to_steamID(steamID):
    ...
    if re.search(r"^STEAM_", id_str): # Already a steamID
        return id_str

    elif re.search(r"^\[.*\]$", id_str): # If passed steamID3
...
def to_steamID3(steamID):
    ...
    if re.search(r"^\[.*\]$", id_str): # Already a steamID3
        return id_str

    elif re.search(r"^STEAM_", id_str): # If passed steamID
